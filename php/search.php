	<?php 
	    ...
	    if($_SERVER["REQUEST_METHOD"] == "POST") {	
	    	if(isset($_POST["search_submit"]) ){
	    		...
	    		if($searchPlace == "Offer"){
	    			if($company_name != ""){
				    	$result = mysqli_query($db,"SELECT * FROM company WHERE company_name='$company_name'");
				    	$array = mysqli_fetch_assoc($result);
				    	$company_username = $array['user_name'];
			    	
				    	if($job_title != ""){
				    		$searchRes = mysqli_query($db,"SELECT * FROM joboffer WHERE job_title = '$job_title' AND user_name='$company_username'");
				    		$result_number = mysqli_num_rows($searchRes);

				    	}
				    	else{
				    		$searchRes = mysqli_query($db,"SELECT * FROM joboffer WHERE user_name='$company_username'");
				    		$result_number = mysqli_num_rows($searchRes);
				    	}
			    	}
				    else if($job_title != ""){
				    	$searchRes = mysqli_query($db,"SELECT * FROM joboffer WHERE job_title = '$job_title'");
				    	$result_number = mysqli_num_rows($searchRes);
				    }
				    else{
				    	$searchRes = mysqli_query($db,"SELECT * FROM joboffer");
				    	$result_number = mysqli_num_rows($searchRes);
				    }
	    		}
	    		else if($searchPlace == "Tender"){
	    			if($company_name != ""){
				    	$result = mysqli_query($db,"SELECT * FROM company WHERE company_name='$company_name'");
				    	$array = mysqli_fetch_assoc($result);
				    	$company_username = $array['user_name'];
				    	...
				    	if($job_title != ""){
				    		$searchRes = mysqli_query($db,"SELECT * FROM tender WHERE job_title = '$job_title' AND user_name='$company_username'");
				    		$result_number = mysqli_num_rows($searchRes);

				    	}
				    	else{
				    		$searchRes = mysqli_query($db,"SELECT * FROM tender WHERE user_name='$company_username'");
				    		$result_number = mysqli_num_rows($searchRes);
				    	}
			   		}
				    else if($job_title != ""){
				    	$searchRes = mysqli_query($db,"SELECT * FROM tender WHERE job_title = '$job_title'");
				    	$result_number = mysqli_num_rows($searchRes);
				    }
				    else{
				    	$searchRes = mysqli_query($db,"SELECT * FROM tender");
				    	$result_number = mysqli_num_rows($searchRes);
				    }
	    		}	    
	    	}
								...
								if($result_number > 0){
									while($row = mysqli_fetch_assoc($searchRes))
									{
										if($searchPlace == "Offer"){
											...
											$tender_end = $row['end_date_offer'];
											$job_end = $row['end_date_job'];
											$tender_end = strtotime("$tender_end");  
											$current = date("D M d, Y G:i"); 
											$current = strtotime("$current");

											$diff = abs($tender_end - $current);  
				 								...
				 								<div class=\"single-post d-flex flex-row\">
													<div class=\"details\">
														<div class=\"title d-flex flex-row justify-content-between\">
															...
															<form action =\"#\" method = \"post\">
																<button value=\"$tender_id\" href=\"signupuser.php\" class=\"genric-btn primary\" style=\" position:absolute; right: 20px;\" >Make Offer</button>
																</form>
															</ul>
														...
														<p>
															Job Finishing Time: $job_end
														</p>
														<p class=\"date\"> Offer Remaining Time: $years years, $months months, $days days, $hours hours </p>
													</div>
												</div>";
										}
									}
								...
								"
						            <div class=\"single-post d-flex flex-row\">	
										<div class=\"details\">
											<div class=\"title d-flex flex-row justify-content-between\">
												<div class=\"titles\">
													<h4>There is not any job offer!</h4>				
												</div>
											</div>
										</div>
									</div>";
