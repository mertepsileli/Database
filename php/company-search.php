	<?php 
	    ...
	    $result_number = mysqli_num_rows($companies);

	    if($_SERVER["REQUEST_METHOD"] == "POST") {	
	    	$company_name = $_POST['searchname'];
	    	$min_found = $_POST['min_found'];
	    	$max_found = $_POST['max_found'];
	    	if($company_name != ""){
	    		if($min_found != ""){
		    		if($max_found != "")
		    			$companies = mysqli_query($db,"SELECT * FROM company WHERE company_name LIKE '%{$company_name}%' AND year BETWEEN $min_found AND $max_found");
		    		else{
		    			$companies = mysqli_query($db,"SELECT * FROM company WHERE company_name LIKE '%{$company_name}%' AND year > $min_found");
		    		}
	    		}
	    		else if($max_found != ""){
	    			$companies = mysqli_query($db,"SELECT * FROM company WHERE company_name LIKE '%{$company_name}%' AND year < $max_found");
	    		}
	    		else{
	    			$companies = mysqli_query($db,"SELECT * FROM company WHERE company_name LIKE '%{$company_name}%'");
	    		}	
	    	}
	    	else if($min_found != ""){
	    		if($max_found != "")
		    		$companies = mysqli_query($db,"SELECT * FROM company WHERE year BETWEEN $min_found AND $max_found");
		    	else
		    		$companies = mysqli_query($db,"SELECT * FROM company WHERE year > $min_found");
	    	}
	    	else if($max_found != ""){
	    		$companies = mysqli_query($db,"SELECT * FROM company WHERE year < $max_found");
	    	}
	    	$result_number = mysqli_num_rows($companies);
	    }
				...
				<div class="container">
					<div class="row search-page-top d-flex align-items-center justify-content-center">
						<div class="banner-content col-lg-12">
							...
							<form action="#" class="search-form-area" method="post">
								<div class="row justify-content-center form-wrap">
									<div class="col-lg-3 form-cols">
										<div class="default-select" id="default-selects2">
											<input type="text" class="form-control" name="searchname" placeholder="Company Name">
										</div>
									</div>
									<div class="col-lg-3 form-cols">
										<div class="default-select" id="default-selects2">
											<input type="text" class="form-control" name="min_found" placeholder="Min Foundation Year">
										</div>
									</div>
									<div class="col-lg-3 form-cols">
										<div class="default-select" id="default-selects2">
											<input type="text" class="form-control" name="max_found" placeholder="Max Foundation Year">
										</div>
									</div>
									<div class="col-lg-2 form-cols">
									    <button type='submit' name="search_submit" class="genric-btn primary-border e-large">Search</button>
									</div>								
								</div>
							</form>	
							<p class="text-white">
								<?php 
									echo "$result_number Results found";
								?>	
							</p>
						</div>											
					</div>
					...
					<div class="row justify-content-center d-flex">
						<div class="col-lg-8 post-list">
							...
							while($row = mysqli_fetch_assoc($companies)){
								...
								$company_username = $row['user_name'];
								$country = $row['country'];
								$city = $row['city'];
								$year = $row['year'];
								...
								<div class=\"details\">
									<div class=\"title d-flex flex-row justify-content-between\">
										<div class=\"titles\">
											<a href=\"companyProfile.php?username=$company_username\"><h4>$company_name</h4></a>
										</div>
									</div>
									<p class=\"address\"><span class=\"lnr lnr-map\"></span> Country: $country </p>
									<p class=\"address\"><span class=\"lnr lnr-database\"></span> City: $city </p>
									<p class=\"address\"><span class=\"lnr lnr-database\"></span> Year: $year </p>
								</div>
							</div>";
							}		
							?>	
